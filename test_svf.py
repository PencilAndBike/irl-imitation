...
PARSER.add_argument('-t', '--n_trajs', default=1000, type=int, help='number of expert trajectories')
PARSER.add_argument('-l', '--l_traj', default=10, type=int, help='length of expert trajectory')
PARSER.add_argument('--rand_start', dest='rand_start', action='store_false',
...
class GWExperiment(object):
  def __init__(self, gamma=0.9, act_rand=0.3, r_max=1, h=10, w=10, n_trajs=100, l_traj=20, rand_start=False,
  ...
  def generate_trajs(self, policy):
    ...
    for i in range(self._n_trajs):
      if self._rand_start:
        ...
        assert self._n_trajs == self._h * self._w
        start_pos = self._gw.idx2pos(i)
      episode = []
      ...
      for _ in range(self._l_traj):
        # cur_state, action, next_state, reward, is_done = self._gw.step(int(policy[self._gw.pos2idx(cur_state)]))
        cur_state, action, next_state, reward, is_done = self._gw.step(int(policy[self._gw.pos2idx(next_state)]))
        episode.append(
          Step(cur_state=self._gw.pos2idx(cur_state), action=action, next_state=self._gw.pos2idx(next_state), reward=reward,
               done=is_done))
        if is_done:
  ...
  def get_svf(self, policy):
    trajs = self.generate_trajs(policy)
    dp_svf = compute_state_visition_freq(self._P_a, self._gamma, trajs, policy)
    mc_svf = demo_svf(trajs, self._h*self._w)
    diff_svf = dp_svf - mc_svf
    print dp_svf
    print mc_svf
    print diff_svf
    print np.sum(diff_svf*diff_svf)
    
    
  def run(self):
    self._values_gt, self._policy_gt = value_iteration.value_iteration(self._P_a, self._rewards_gt, GAMMA, error=0.01,
                                                                       deterministic=True)
    self.get_svf(self._policy_gt)

if __name__ == "__main__":
  print RAND_START
  print TERMINAL
  gwExperiment = GWExperiment(GAMMA, ACT_RAND, R_MAX, H, W, N_TRAJS, L_TRAJ, True, LEARNING_RATE,
                              N_ITERS, SAVE_DIR, EXP_NAME, N_EXP, terminal=TERMINAL)
  gwExperiment.run()